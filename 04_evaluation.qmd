...
ax.set_title('Deep Neural Network', fontsize=14, fontweight='bold')
...
Forest aboveground biomass (AGB) regression is a more complex modelling task compared to classifying dominant tree species.

We begin by evaluating DGCNN model trained with hyperparameters that achieved the best validation performance. 

**Code below include a function for loading regression predictions and converting them from Z-score to original scale.**
...
We also need to load the observed AGB values from the `labels.csv`.

**Below we generate an observed predicted scatterplot including regression metrics for DGCNN.** 
...
We can see that OCNN achieved a better fit compared to DGCNN, reducing RMSE and bias and improving R².
...
Due to the hiogh degree of flexibility in DNN architectures and training strategies, there are many other potential avenues for improving model performance that can be explored. These include but are not limited to:
...
[Safonova et al. 2023](https://doi.org/10.1016/j.jag.2023.103569) provides additional strategies for improving DNN performance for remote sensing applications in scenarios with small datasets.
