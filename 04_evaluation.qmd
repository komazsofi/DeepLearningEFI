...
#| code-fold: true
# helper functions
def visualize_side_by_side(plot_id, pc_dir):
    row = df[df.plot_id == plot_id].iloc[0]
    gt = row.dom_sp_type
    pred = row.pred_dom_sp_type

    xyz = np.load(pc_dir / f"{plot_id}.npy")[:, :3]
    ...
    # GT view (blue)
    plotter.subplot(0, 0)
    plotter.add_points(cloud, color="blue", point_size=3)
    plotter.add_title(f"GT = {gt}")

    # Prediction view (orange)
    plotter.subplot(0, 1)
    plotter.add_points(cloud, color="orange", point_size=3)
    plotter.add_title(f"Pred = {pred}")

    return plotter.show()


def sample_cases(df, n=5, seed=42):
    random.seed(seed)

    correct_df = df[df.correct]
    wrong_df = df[~df.correct]

    n_correct = min(n, len(correct_df))
    n_wrong = min(n, len(wrong_df))

    sample_correct = correct_df.sample(n_correct, random_state=seed)
    sample_wrong = wrong_df.sample(n_wrong, random_state=seed)

    return sample_correct, sample_wrong


def visualize_samples(sample_df, pc_dir, title_prefix=""):
    for _, row in sample_df.iterrows():
        pid = row.plot_id
        print(f"{title_prefix} {pid}")
        visualize_side_by_side(pid, pc_dir)
...
sample_correct, sample_wrong = sample_cases(df, n=4)

print("===== RANDOM CORRECT SAMPLES =====")
visualize_samples(sample_correct, pc_dir, "CORRECT")

print("===== RANDOM INCORRECT SAMPLES =====")
visualize_samples(sample_wrong, pc_dir, "WRONG")
```

Result:

![correct](images/04_eval/output_correct.png)
![incorrect](images/04_eval/output_incorrect.png)
