...
class CustomTransform:
    ...
    def preprocess(self, sample: dict, idx: int):
        ...
        xyz = sample.pop('points')

        if (self.augment) & (self.downsample_prop > 0.0):
            xyz = rand_pc_downsample(xyz, self.downsample_prop)

        xyz = torch.from_numpy(xyz).float()
            
        # Retain non-normalized coordinates height for use as feature
        height = xyz[:, 2:3].clone()

        sample['points'] = Points(xyz, features=height)
...
class CustomCollateBatch:
  ...
  """
    ...
    def __init__(self, batch_size: int, target_len: int, merge_points: bool = False):
        ...
        for key in batch[0].keys():
            ...
            if 'target' in key:
                if self.target_len > 1:
                    ...
                    target_reshape = torch.reshape(target_reshape, (num_samples, self.target_len))
                    outputs['target'] = target_reshape
                else:
                    outputs['target'] = torch.stack(outputs['target']).unsqueeze(-1)
