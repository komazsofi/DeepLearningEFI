...
2. How big each batch is

3. How many background workers load data in parallel (make sure batches are efficiently provided to the GPU workers).
...
## The Model Wrapper: EfiModelModule ([efi_modelmodule.py](https://github.com/Brent-Murray/DeepLearningEFI/tree/main/src/models/efi_modelmodule.py))
...
Using the `EfiModelModule` allows us to decouple the research logic from the engineering boilerplate. @tbl-wrapper-comparison summarizes the main differences between a standard PyTorch training workflow and the Lightning-based wrapper used in this work.
